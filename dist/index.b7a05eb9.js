...
inputEl.addEventListener("keydown", (event)=>{
    if (event.key === "Enter" && !event.isComposing) searchMovies(inputEl.value);
});
...
const io = new IntersectionObserver((entries)=>{
    entries.forEach((entry)=>{
        ...
        if (entry.isIntersecting) searchMovies(inputEl.value, currnetPage + 1);
    });
});
...
},{}]},["84Rv8","jeorp"], "jeorp", "parcelRequirea3da")
